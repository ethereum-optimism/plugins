...
export const getSigners = async (
  ...
  const signers = accounts.map((account: string) =>
    hre.l2ethers.provider.getSigner(account)
  )
...
const addGasToAbiMethodsIfNecessary = (
...
const getContractFactoryByAbiAndBytecode = async (
  hre: HardhatRuntimeEnvironment,
  abi: any[],
...
const getContractFactoryByName = async (
  hre: HardhatRuntimeEnvironment,
  contractName: string,
  signerOrOptions?: ethers.Signer | FactoryOptions
) => {
...
export const collectLibrariesAndLink = async (
  artifact: Artifact,
  libraries: Libraries
) => {
...
const linkBytecode = (artifact: Artifact, libraries: Link[]): string => {
